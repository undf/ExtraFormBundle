//This directive add some dirty hacks to the datepicker directive from the angular-bootstrap-ui project
//because the original directive doesn´t work properly with bootstrap 3
//TODO: Check the original project and once there is a stable version for bootstrap 3
//get rid of this directive and use the original one
uFormUtils.directive( 'uDatepicker', ['dateFilter', '$parse', 'datepickerConfig', function (dateFilter, $parse, datepickerConfig) {
  return {
    restrict: 'EA',
    replace: true,
    scope: {
    ...
    link: function(scope, element, attrs) {
      ...
      scope.$watch('model', function ( dt, olddt ) {
        ...
        if ( angular.isDate(dt) ) {
          ...
          scope.output = dateFilter(selected, 'dd/MM/yyyy');
        }
...
//        }
      ...
      scope.$watch('mode', function() {
        if(!angular.isDate(scope.model)) {
          ...
          if(parts.length === 3)
            scope.model = new Date(parseInt(parts[2]), parseInt(parts[1]) - 1, parseInt(parts[0]));
        }
        updateShowWeekNumbers();
        refill();
      });
